...
(/** @lends liquid.helper */function(helper) {
	...
	helper.oauth = {
		...
		authorize: function(callBack)
		{
			...
			//window.open(authUri, {showLocationBar : true});

			ref = window.open(authUri, {showLocationBar : true});


			ref.addEventListener('loadstop', function(event) {

					var crntUrl = event.url;

					//alert(crntUrl);

					if (crntUrl.indexOf("localhost/?state=undefined&code=") > 0){
						alert(crntUrl);
						$this.onAuthUrlChange(crntUrl);
					}
			...
			//addEventListener('loadstop', $this.onAuthUrlChange(event.url));
			ref.addEventListener('exit', $this.onAuthClose(crntUrl));
		 ...
		 */
		onAuthUrlChange: function(loc) {
			var $this = helper.oauth;
			var ref = window.open(loc, {showLocationBar : true});
			//ref.removeEventListener('loadstart', function(event); );
			alert('HELLO!!');
			if(loc.indexOf("code=") != -1) {
				$this.requestStatus = $this.status.SUCCESS;

				/* Store the authCode temporarily */
				$this.authCode = $this.getParameterByName("code", loc);
		    ...
		    else if(loc.indexOf("error=") != -1)
		    {
		    	...
		    	$this.errorMessage = $this.getParameterByName("error", loc);
