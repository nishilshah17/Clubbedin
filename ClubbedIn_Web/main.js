    ...
    function getLogo() {
        ...
        $.ajax({
            ...
            success: function (data) {
                var json = jQuery.parseJSON(data);
                if(json == null) {
                    $('#clubimage').append('<img width=25% height=25% src="images/unknown.jpg" align="left">');
                } else {
                     $('#clubimage').append('<img width=25% height=25% src=\"' + json.logo + '\" align="left">');
                }
            }
        });
    }
    ...
    function getEventData(id, num) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                var vlink = "http://www.maps.google.com/?q="+json.venue;

                econt.append(
				    $('<strong>Venue: </strong><a>'+json.venue+'</a>').click(function() {
						window.open(vlink, '_blank');
    			}));
                $("#venuelink").attr("href", "http://www.google.com/");
                curEvent = id;
            }
        });
        addSwitch(id);
